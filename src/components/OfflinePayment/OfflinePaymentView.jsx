...
import PaymentCollectionStep from "./PaymentCollectionStep";
import PaymentSuccessStep from "./PaymentSuccessStep";
import FullyPaidPropertyDetails from "./FullyPaidPropertyDetails";
...
export default function OfflinePaymentView({ onNavigateBack }) {
  ...
  const criteriaNavItems = [
    ...
    { key: "etc", label: t("etc", "Etc."), icon: "description" },
  ];
    ...
    return (
      <PropertyVerifyView
        ...
        onProceedToPayment={(info) => {
          ...
          setStep(3); // Advance to Step 3: Payment Collection
        }}
        onCancel={() => {
          setStep(1);
        }}
    ...
    return (
      <PaymentCollectionStep
        ...
        onConfirmPayment={(result) => {
          ...
          setStep(4); // Advance to Step 4: Digital Receipt & Success
        }}
      />
    );
  }
  ...
  // Step 6: Fully Paid Property Details (Only for successful Total Payment)
  if (step === 6) {
    return (
      <FullyPaidPropertyDetails
        propertyData={selectedProperty || transactionResult?.propertyData}
        onReturnToSearch={() => {
          setStep(1);
          setSelectedProperty(null);
          setPaymentDetails(null);
  ...
  return (
    <div className="w-full p-margin-mobile md:p-margin-desktop space-y-stack-lg animate-in fade-in duration-300">
